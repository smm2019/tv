...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def homeVideoContent(self):
		rsp = self.fetch('https://www.physkan.com/',headers=self.header)
		#https://www.physkan.com/
		htmlTxt=rsp.text
	...
	def categoryContent(self,tid,pg,filter,extend):
		...
		if tid=='new' and int(pg)>1:
			return result
		url='https://www.physkan.com/vodshow/{0}/page/{1}.html'.format(tid,pg)
		if tid=='new':
			url='https://www.physkan.com/label/new.html'
	...
	def searchContent(self,key,quick):
		Url='https://www.physkan.com/phsch.html?wd={0}'.format(urllib.parse.quote(key))
		rsp = self.fetch(Url,headers=self.header)
		htmlTxt=rsp.text
	...
	def playerContent(self,flag,id,vipFlags):
		result = {}
		parse=1
		Url='https://www.physkan.com{0}'.format(id)
		rsp = self.fetch(Url,headers=self.header)
		htmlTxt = rsp.text
	...
	header = {
		"Referer": 'https://www.physkan.com/',
		'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36',
		'Host': 'www.physkan.com'
