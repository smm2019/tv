	...
	def detailContent(self,array):
		...
		try:
			...
			for vod in jsonList:
				...
				vod_year=vod['vod_time']
				vod_actor=vod['vod_actor']
				vod_content=vod['vod_content']
				vod_director='/'.join(vod['vod_director'])
				type_name='/'.join(vod['type_name'])
				vod_area='/'.join(vod['vod_area'])
		except :
			vod_play_url=[]
		vod = {
			"vod_id":array[0],
			...
			"vod_pic":logo,
			"type_name":type_name,
			"vod_year":vod_year[0:4] if len(vod_year)>4 else '',
			"vod_area":vod_area,
			...
			if len(idTxt)>1:
				...
				for vod in jsonList:
					...
					videos.append({
						...
						"vod_name":title,
						"vod_pic":img,
						"vod_year":self.custom_RegexGetText(Text=vod_year,RegexText=r'([0-9]{4})-',Index=1),
						"vod_remarks":remarks
					})
		return videos
		#访问网页
